...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
    ...
    const navItems = [
        ...
        { icon: PlayCircle, label: "Academy", path: "/academy" },
                ...
                <SidebarMenu>
                            ...
                            <DropdownMenuTrigger asChild>
                                ...
                                >
                                    ...
                                    <div className="flex flex-1 items-center justify-between group-data-[collapsible=icon]:hidden">
                                        <span className="font-bold text-xs uppercase tracking-widest">{currentLang.label}</span>
                                        <ChevronUp className="size-3 h-4 w-4 opacity-50" />
                                    </div>
                                </SidebarMenuButton>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="start" side="right" className="w-56 p-2 rounded-2xl shadow-2xl border-border/50 backdrop-blur-xl bg-background/95 animate-in slide-in-from-left-2 duration-300">
                                ...
                                {LANGUAGES.map((lang) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <span className="text-xl leading-none transition-transform group-hover/item:scale-110">{lang.flag}</span>
                                            <span className="text-xs uppercase tracking-wider">{lang.label}</span>
                                        </div>
                                        {language === lang.code && <div className="w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_8px_rgba(var(--primary),0.5)]" />}
                                    </DropdownMenuItem>
                                ))}
                            </DropdownMenuContent>
                        </DropdownMenu>
                    ...
                    <SidebarMenuItem>
                        ...
                        >
                            ...
                            <span className="font-bold text-xs uppercase tracking-widest">Theme</span>
                        </SidebarMenuButton>
                    </SidebarMenuItem>
